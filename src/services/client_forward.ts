...
export class ClientForward implements Client {
  ...
  public async send(payload: any, options: any) {
    ...
    if (isUnoId(payload?.message_id)) {
      logger.debug(
        "Ignore status message %s because is internal unoapi message",
        payload["message_id"],
      );
      ...
      logger.debug(
        "Ignore status message %s without message_id",
        payload.status,
      );
      return { ok: { success: true }, error: undefined };
    }
    ...
    const endpoint = options.endpoint && payload.type
      ? options.endpoint
      : "messages";
    const url =
      `${config.webhookForward.url}/${config.webhookForward.version}/${config.webhookForward.phoneNumberId}/${endpoint}`;
    logger.debug(
      `Send url ${url} with headers %s and body %s`,
    ...
    try {
      ...
      logger.error(
        error,
        `Error on send to url ${url} with headers %s and body %s`,
        JSON.stringify(headers),
        body,
      );
      throw error;
